-- ── Run-once guard ────────────────────────────────────────────
if getgenv().__BrightsideSourceLoaded then
    warn("[Brightside] Source already loaded, skipping.")
    return
end
getgenv().__BrightsideSourceLoaded = true
...
task.spawn(function()
    ...
    local function checkMod(Player)
        if getgenv().Brightside.Global["Mod Detector"] then
...
local function getCameraSmoothness(distance)
    ...
    if not cfg.Enabled then
        return getgenv().Brightside['Camera Aimbot']['Smoothing'].X,
               getgenv().Brightside['Camera Aimbot']['Smoothing'].Y
    end
...
local function isMouseInBoxFOV(hitbox)
    ...
    local function axis(o, d, minB, maxB)
        ...
        local t1 = (minB-o)/d; local t2 = (maxB-o)/d
...
local function isMouseInTriggerHitbox()
    ...
    if result and result.Instance then
        for _, partName in ipairs(parts) do
            if result.Instance.Name == partName then return true end
        end
    end
    return false
end
    ...
    if mode == "Basic" then
        for _, part in parts do
            ...
            if s.Z > 0 then
                ...
                if dist < bestDist then bestDist=dist; bestPart=part; bestPos=closest end
            end
        end
    else
        ...
        for _, part in parts do
            local size = part.Size; local half = size*0.5
            local cframe = part.CFrame; local scaledHalf = half*(1-scaleFactor)
            for z = 1, density do
                ...
                for y = 1, density do
                    ...
                    for x = 1, density do
                        ...
                        if s.Z > 0 then
                            local dist = (s.X-mouseX)^2 + (s.Y-mouseY)^2
                            if dist < bestDist then bestDist=dist; bestPart=part; bestPos=worldPos end
                        end
                    end
                end
            end
...
local function updatetargetline()
    ...
    if not cfg['enabled'] then targetline.Visible=false; return end
    if not targetPlayer or not targetPlayer.Character then targetline.Visible=false; return end
    ...
    if onscreen and screenpos.Z > 0 then
        ...
        targetline.Color   = isMouseInSilentFOV() and cfg['in fov'] or cfg['regular']
...
local function getBestTarget()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if dist2D < closestDist then closestDist=dist2D; closestPlayer=player end
    end
...
local function destroyTargetUI()
    if targetUIUpdateConn then targetUIUpdateConn:Disconnect(); targetUIUpdateConn=nil end
    if targetUI then targetUI:Destroy(); targetUI=nil end
    targetline.Visible = false
...
local function showTargetUI(player)
    ...
    stroke.Color=Color3.fromRGB(210,20,20); stroke.Thickness=1.8; stroke.Transparency=0.1; stroke.Parent=frame
    local PFP_SIZE = 48
    local pfpBorder = Instance.new("Frame")
    pfpBorder.Size=UDim2.new(0,PFP_SIZE+4,0,PFP_SIZE+4); pfpBorder.Position=UDim2.new(0,6,0.5,-(PFP_SIZE/2+2))
    pfpBorder.BackgroundColor3=Color3.fromRGB(210,20,20); pfpBorder.BorderSizePixel=0; pfpBorder.ZIndex=2; pfpBorder.Parent=frame
    local pfpBorderCorner=Instance.new("UICorner"); pfpBorderCorner.CornerRadius=UDim.new(0,6); pfpBorderCorner.Parent=pfpBorder
    local pfpImg = Instance.new("ImageLabel")
    pfpImg.Name="PFP"; pfpImg.Size=UDim2.new(0,PFP_SIZE,0,PFP_SIZE); pfpImg.Position=UDim2.new(0,2,0,2)
    pfpImg.BackgroundColor3=Color3.fromRGB(22,22,32); pfpImg.BackgroundTransparency=0
    pfpImg.Image=""; pfpImg.ScaleType=Enum.ScaleType.Crop; pfpImg.ZIndex=3; pfpImg.Parent=pfpBorder
    local pfpCorner=Instance.new("UICorner"); pfpCorner.CornerRadius=UDim.new(0,4); pfpCorner.Parent=pfpImg
    local TX = PFP_SIZE + 18
    local lockedLabel = Instance.new("TextLabel")
    lockedLabel.Size=UDim2.new(0,110,0,18); lockedLabel.Position=UDim2.new(0,TX,0,8)
    lockedLabel.BackgroundTransparency=1; lockedLabel.Text="LOCKED"
    lockedLabel.TextColor3=Color3.fromRGB(255,45,45); lockedLabel.TextSize=13
    lockedLabel.Font=Enum.Font.GothamBold; lockedLabel.TextXAlignment=Enum.TextXAlignment.Left
    lockedLabel.ZIndex=2; lockedLabel.Parent=frame
    local lockedStroke=Instance.new("UIStroke")
    lockedStroke.Color=Color3.fromRGB(255,0,0); lockedStroke.Thickness=2.5; lockedStroke.Transparency=0.0
    lockedStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Contextual; lockedStroke.Parent=lockedLabel
    local nameLabel = Instance.new("TextLabel")
    nameLabel.Size=UDim2.new(0,150,0,16); nameLabel.Position=UDim2.new(0,TX,0,22)
    nameLabel.BackgroundTransparency=1; nameLabel.Text=dispName
    nameLabel.TextColor3=Color3.fromRGB(210,210,210); nameLabel.TextSize=12
    nameLabel.Font=Enum.Font.GothamBold; nameLabel.TextXAlignment=Enum.TextXAlignment.Left
    nameLabel.TextTruncate=Enum.TextTruncate.AtEnd; nameLabel.ZIndex=2; nameLabel.Parent=frame
    local hpLabel = Instance.new("TextLabel")
    hpLabel.Name="HPLabel"; hpLabel.Size=UDim2.new(0,62,0,16); hpLabel.Position=UDim2.new(0,TX,0,34)
    hpLabel.BackgroundTransparency=1; hpLabel.Text="HP --"
    hpLabel.TextColor3=Color3.fromRGB(85,225,95); hpLabel.TextSize=12
    hpLabel.Font=Enum.Font.GothamBold; hpLabel.TextXAlignment=Enum.TextXAlignment.Left
    hpLabel.ZIndex=2; hpLabel.Parent=frame
    local armLabel = Instance.new("TextLabel")
    armLabel.Name="ARMLabel"; armLabel.Size=UDim2.new(0,80,0,16); armLabel.Position=UDim2.new(0,TX+65,0,34)
    armLabel.BackgroundTransparency=1; armLabel.Text="ARM --"
    armLabel.TextColor3=Color3.fromRGB(55,195,255); armLabel.TextSize=12
    armLabel.Font=Enum.Font.GothamBold; armLabel.TextXAlignment=Enum.TextXAlignment.Left
    armLabel.ZIndex=2; armLabel.Parent=frame
    local distLabel = Instance.new("TextLabel")
    distLabel.Name="Distance"; distLabel.Size=UDim2.new(0,88,0,16); distLabel.Position=UDim2.new(1,-94,0,6)
    distLabel.BackgroundTransparency=1; distLabel.Text="-- studs"
    distLabel.TextColor3=Color3.fromRGB(175,175,185); distLabel.TextSize=11
    distLabel.Font=Enum.Font.Gotham; distLabel.TextXAlignment=Enum.TextXAlignment.Right
    distLabel.ZIndex=2; distLabel.Parent=frame
    ...
    local function updateStats()
        if not targetPlayer or not targetPlayer.Character then destroyTargetUI(); return end
        local char  = targetPlayer.Character
        local hrp   = char:FindFirstChild("HumanoidRootPart")
        ...
        if hum then
            ...
            hpLabel.Text = "HP "..hp
            if pct > 0.6 then hpLabel.TextColor3=Color3.fromRGB(85,225,95)
            elseif pct > 0.3 then hpLabel.TextColor3=Color3.fromRGB(240,200,50)
            else hpLabel.TextColor3=Color3.fromRGB(220,60,60) end
        end
        ...
        if armVal ~= nil then
            ...
            armLabel.Text="ARM --"; armLabel.TextColor3=Color3.fromRGB(100,100,120)
        end
    end
    updateStats()
    ...
    for userid, esp in pairs(esplabels) do
        ...
        if player.Character and player.Character.Parent and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Head") then
            ...
            if onscreen and esppos.Z > 0 then
                ...
                esp.nametag.Text = getgenv().Brightside['esp']['use display name'] and player.DisplayName or player.Name
                esp.nametag.Color = (targetPlayer and targetPlayer.Character and player.Character==targetPlayer.Character)
                    and getgenv().Brightside['esp']['target color']
                    or getgenv().Brightside['esp']['color']
...
local function applygun(tool, name)
    ...
    if sa2 then local pf=sa2:FindFirstChild("GunHandleParticle"); if pf then local ps=pf:FindFirstChild(name); if ps then local pe=ps:FindFirstChild("ParticleEmitter"); if pe then for _,ex in pairs(handle:GetChildren()) do if ex:IsA("ParticleEmitter") then ex:Destroy() end end; pe:Clone().Parent=handle end end end end
...
local function setuptool(tool)
    ...
    tool.Unequipped:Connect(function()
        if tool.Name=="[Knife]" then
            ...
            if data.sounds then for _,s in pairs(data.sounds) do if s and s.Parent then s:Destroy() end end; data.sounds={} end
            local mesh=tool:FindFirstChild("Default")
            if mesh then
                for _,v in pairs(mesh:GetChildren()) do
...
local function oncharrapidfire(char)
    ...
    char.ChildAdded:Connect(function(tool)
        if tool:IsA("Tool") and getgenv().Brightside['rapid fire']['enabled'] then patchtool(tool) end
    end)
end

local function rapidfire()
    if not getgenv().Brightside['rapid fire']['enabled'] then isfiring=false; return end
    if not isfiring then return end
    if tick()-lastrapidfire < getgenv().Brightside['rapid fire']['delay'] then return end
    local gun = getrapidgun(); if not gun then return end
...
if localPlayer.Character then oncharrapidfire(localPlayer.Character) end

-- ── Movement / Ammo / Cooldown ────────────────────────────────
        ...
        for _, conn in pairs(getconnections(tool.Activated)) do
            ...
            if ok and info then
                for i = 1, info.nups do
                    ...
                    if ok2 and type(val)=="number" and val>0 then pcall(debug.setupvalue, conn.Function, i, 0) end
                end
            end
        end
                    ...
                    for _, conn in ipairs(conns) do
                        ...
                        if ok2 and info then
                            for i = 1, info.nups do
                                ...
                                if ok3 and type(val)=="number" and val>0 then pcall(debug.setupvalue, conn.Function, i, 0) end
                            end
                        end
                    end
                end
...
RunService.RenderStepped:Connect(function()
    ...
    if infammoenabled and getgenv().Brightside['inf ammo']['enabled'] then
        if gun then local ammo=gun:FindFirstChild("Ammo"); if ammo then ammo.Value=30 end end
    end
    if nocooldownenabled and getgenv().Brightside['no cooldown']['enabled'] then
        if gun then nukeToolCooldowns(gun) end
    end
    if superjumpenabled and getgenv().Brightside['super jump']['enabled'] then
        local hum = localPlayer.Character and localPlayer.Character:FindFirstChild("Humanoid")
        if hum and hum.JumpPower ~= getgenv().Brightside['super jump']['jump power'] then
    ...
    if getgenv().Brightside['Speed Modifications']['Enabled'] and speedenabled then
        local hum = localPlayer.Character and localPlayer.Character:FindFirstChild("Humanoid")
        if hum then hum.WalkSpeed = 16 * getgenv().Brightside['Speed Modifications']['Normal']['Multiplier'] end
    end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if key == Enum.KeyCode[selectBind] and targetMode=='Select' then
        if not selectPressed then
            ...
            if targetPlayer then
                targetPlayer=nil; destroyTargetUI()
                pcall(function()
                    if targetCache.Hitbox    then targetCache.Hitbox:Destroy()    end
                    if targetCache.Box       then targetCache.Box:Destroy()       end
                    if targetCache.Trigger   then targetCache.Trigger:Destroy()   end
        ...
        if not camPressed then
            ...
            if mode=="Toggle" then
                ...
                if camLockActive then
                    ...
                    if camLockTarget and camLockTarget.Character then camLockPart=getCamlockBodyPart(camLockTarget.Character) end
    ...
    if key == Enum.KeyCode[speedBind] then
        if not speedPressed then
            speedPressed=true
            if getgenv().Brightside['Speed Modifications']['Enabled'] then speedenabled=not speedenabled end
        end
    end

    if key == Enum.KeyCode[getgenv().Brightside["Binds"]['Super Jump']] then
        if getgenv().Brightside['super jump']['enabled'] then superjumpenabled=not superjumpenabled end
    end
    if key == Enum.KeyCode[getgenv().Brightside["Binds"]['Inf Ammo']] then
        if getgenv().Brightside['inf ammo']['enabled'] then infammoenabled=not infammoenabled end
    end
    if key == Enum.KeyCode[getgenv().Brightside["Binds"]['No Cooldown']] then
        if getgenv().Brightside['no cooldown']['enabled'] then nocooldownenabled=not nocooldownenabled end
    end
    if key == Enum.KeyCode[getgenv().Brightside["Binds"]['ESP']] then
        ...
        if getgenv().Brightside['esp']['enabled'] then
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    if not esplabels[player.UserId] then addesp(player) end
                end
            end
        end
    end
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        if getgenv().Brightside['rapid fire']['enabled'] then
            local gun = getrapidgun(); if gun then isfiring=true end
        end
    end
...
UserInputService.InputEnded:Connect(function(input)
    ...
    if input.KeyCode == Enum.KeyCode[getgenv().Brightside["Binds"].Triggerbot] then
        if getgenv().Brightside['Trigger Bot'].Settings.Mode=="Hold" then triggerHold=false end
        triggerPressed=false
    end
    if input.KeyCode == Enum.KeyCode[getgenv().Brightside["Binds"].Select] then selectPressed=false end
    if input.KeyCode == Enum.KeyCode[getgenv().Brightside["Binds"].Speed]  then speedPressed=false end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    local root = targetPlayer.Character:FindFirstChild("HumanoidRootPart"); if not root then return end
    ...
    local function applySilentFOV()
        ...
        if not targetCache.Hitbox then
            ...
            targetCache.Hitbox.Transparency=1; targetCache.Hitbox.CanQuery=false; targetCache.Hitbox.Parent=Workspace
        end
        ...
        local offset=Vector3.new((silentFOV.xRight-silentFOV.xLeft)/2,(silentFOV.yUpper-silentFOV.yLower)/2,(silentFOV.zRight-silentFOV.zLeft)/2)
        targetCache.Hitbox.Size=size; targetCache.Hitbox.CFrame=CFrame.new(basePos+facing:VectorToWorldSpace(offset))*facing
    ...
    local function applyTriggerFOV()
        ...
        if not targetCache.Trigger then
            ...
            targetCache.Trigger.Transparency=1; targetCache.Trigger.CanQuery=false; targetCache.Trigger.Parent=Workspace
        end
        local pred=getgenv().Brightside['Trigger Bot'].Prediction
        local predPos=root.Position
        if root.Velocity.Magnitude>1 then predPos=predPos+root.Velocity*Vector3.new(pred.X,pred.Y,pred.Z) end
        ...
        local offset=Vector3.new((triggerFOV.xRight-triggerFOV.xLeft)/2,(triggerFOV.yUpper-triggerFOV.yLower)/2,(triggerFOV.zRight-triggerFOV.zLeft)/2)
        local upperPos = upperTorso and upperTorso.Position or predPos
        targetCache.Trigger.Size=size; targetCache.Trigger.CFrame=CFrame.new(upperPos+facing:VectorToWorldSpace(offset))*facing
    ...
    if getgenv().Brightside['Trigger Bot'].Enabled and not leftCtrlHeld then
        ...
        if not targetKnocked then
            ...
            if active then
                local now = tick(); local delay = getTriggerbotDelay()
                ...
                if inRange then
                    ...
                    else hitData=getClosestBodyPart(targetPlayer.Character) end
                    if hitData and hitData.Part then
                        ...
                        if visible then triggerbot(); lastTriggerTime=now end
                    end
                end
            end
        end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    local root = camLockTarget.Character:FindFirstChild("HumanoidRootPart"); if not root then return end
    ...
    local isFP = zoom < 1; local isTP = zoom >= 1
    ...
    local newPart = getCamlockBodyPart(camLockTarget.Character); if not newPart then return end
    ...
    if humanize.Bezier and humanize.Enabled then
        ...
        local t = (tick()%1); local p1=camera.CFrame.Position; local p2=control; local p3=targetPos
    ...
    if camcfg.FOV['Show FOV'] then
        if not camFOVCircle then
            camFOVCircle=Drawing.new("Circle"); camFOVCircle.Thickness=1.5; camFOVCircle.Filled=false
            camFOVCircle.Transparency=1; camFOVCircle.Color=Color3.new(1,1,1); camFOVCircle.Visible=true
        end
        local mouseLoc = UserInputService:GetMouseLocation()
        camFOVCircle.Position=Vector2.new(mouseLoc.X,mouseLoc.Y); camFOVCircle.Radius=radius
    elseif camFOVCircle then
        camFOVCircle:Remove(); camFOVCircle=nil
    end
end)
...
originalIndex = hookmetamethod(game, "__index", function(t, k)
    ...
    local alwaysHit    = forceHit or inFOV
    ...
    if not hitData or not hitData.Part then return originalIndex(t, k) end
    ...
    if (#args==0) or (args[1]==-0.05 and args[2]==0.05) or (args[1]==-0.1) or (args[1]==-0.05) then
        if getgenv().Brightside['spread modifications']['enabled'] then
            if getgenv().Brightside['spread modifications']['specific weapons']['enabled'] then
                local tool = localPlayer.Character and localPlayer.Character:FindFirstChildOfClass("Tool")
                if tool then
                    local wname = tool.Name; local found = false
                    ...
                    if found then return oldrandom(...) * (getgenv().Brightside['spread modifications']['amount']/100) end
                end
            else
