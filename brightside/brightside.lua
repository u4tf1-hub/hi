...
-- Run-once guard: prevents re-execution if already loaded
if getgenv().__BrightsideLoaded then
    warn("[Brightside] Already running, skipping duplicate execution.")
    return
end
...
-- 
--  TRIGGERBOT (Universal - works in any game)
-- 
local function getCharFromPart(part)
    -- Walk up hierarchy to find character with humanoid
    local current = part
    for _ = 1, 6 do
...
local function applyMovement()
    ...
    if State.SpeedActive and sc['Enabled'] then
        hum.WalkSpeed = tonumber(sc['Value']) or 32
    else
        hum.WalkSpeed = defaultSpeed or 16
    end
    -- Jump Power
    if State.JumpActive and jc['Enabled'] then
        hum.JumpPower = tonumber(jc['Value']) or 100
    else
